...
> **Текущая стадия:** прототип согласован, готов к реализации

---

## История изменений

### v1.3 (29 апреля 2026) — текущая
- **Добавлена функция «Сравнение периодов»** (см. §19 + отдельный документ `SPECIFICATION_period_comparison.md`)
- Прототип: `prototype/period_comparison_simple.html`
- Гибкий выбор двух периодов с date-инпутами и быстрыми шаблонами
- Фильтр активностей с двумя режимами: «Все активности» / «Выбрать»
- Inline-инсайты при наличии асимметрии видов спорта
- URL-параметры для shareable-ссылок
- Отдельный маршрут `/comparison` (рекомендуется как отдельная страница)
...
## 19. Сравнение периодов (★ NEW v1.3)

> **Полная спецификация:** см. отдельный документ `SPECIFICATION_period_comparison.md`
> **Прототип:** `prototype/period_comparison_simple.html`

### 19.1. Краткое описание
Отдельная страница `/comparison` для сравнения активности атлета между двумя произвольными периодами. Позволяет:
- Выбирать любые две даты для каждого периода вручную или через быстрые шаблоны
- Гибко управлять учитываемыми видами спорта (режим «Все» / «Выбрать»)
- Видеть разбивку по видам в каждом периоде с подсветкой исключённых
- Получать сводный результат с дельтой (часы и километры)
- Получать inline-инсайты при асимметрии (виды есть только в одном периоде)
- Шарить ссылки через URL-параметры

### 19.2. Ключевые компоненты
- `PeriodInputBlock` — блок ввода дат с шаблонами быстрого выбора (см. §3.1 в документе сравнения)
- `ActivityFilterBlock` — переключатель `Все / Выбрать` + пилюли видов спорта
- `PeriodComparisonCard` — карточка с разбивкой по видам с подсветкой исключённых
- `ResultCard` — итоговая таблица с дельтой и инсайтом

### 19.3. API
- `GET /api/dashboard/comparison` — новый эндпоинт (см. §6 в документе сравнения)

### 19.4. Acceptance Criteria
- ✅ Все даты можно вводить вручную или через шаблоны
- ✅ Фильтр активностей работает в двух режимах
- ✅ Расчёт пересчитывается мгновенно
- ✅ URL содержит все параметры для шеринга
- ✅ Адаптивная вёрстка для мобильного
- ✅ Inline-инсайты предупреждают об асимметрии

### 19.5. Связанные файлы
- `SPECIFICATION_period_comparison.md` — полное ТЗ функции (~600 строк)
- `prototype/period_comparison_simple.html` — рабочий интерактивный прототип
